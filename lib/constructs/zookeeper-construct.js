...
const reservedIps = ["10.0.0.5", "10.0.0.7", "10.0.0.9"];
class ZookeeperCluster extends cdk.Construct {
  constructor(scope, id, props) {
    ...
    const ipsAndSubnets = Object.keys(subnets).map((subnet) => {
      console.log(subnet);
      return {
        ip: subnets[subnet].ip,
        id: subnets[subnet].id,
    ...
    for (let i = 1; i <= props.num; i++) {
      const zookeeper = new ec2.Instance(this, "zk" + i, {
        ...
        privateIpAddress: `${ipsAndSubnets[i - 1].ip}`,
        vpcSubnets: {
          subnets: [
            ec2.Subnet.fromSubnetAttributes(this, `SubnetFromAttributes${i}`, {
              subnetId: `${ipsAndSubnets[i - 1].id}`,
              availabilityZone: `${ipsAndSubnets[i - 1].az}`,
            }),
          ],
        },
      });
      ...
      zookeeper.userData.addCommands(
        ...
        `sed -i "s,server.1=.*,server.1=${ipsAndSubnets[0].ip}:2888:3888," /home/ubuntu/zookeeper/config/zookeeper.properties`,
        `sed -i "s,server.2=.*,server.2=${ipsAndSubnets[1].ip}:2888:3888," /home/ubuntu/zookeeper/config/zookeeper.properties`,
        `sed -i "s,server.3=.*,server.3=${ipsAndSubnets[2].ip}:2888:3888," /home/ubuntu/zookeeper/config/zookeeper.properties`,
        `cd /home/ubuntu/zookeeper`,
        `sudo bin/zookeeper-server-start.sh -daemon config/zookeeper.properties`,
        `sleep 5`,
