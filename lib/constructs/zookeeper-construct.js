  ...
  constructor(scope, id, props) {
    ...
    for (let i = 1; i <= props.num; i++) {
      const zookeeper = new ec2.Instance(this, "zk" + i, {
        ...
        privateIpAddress: `${props.ipsAndSubnets[i - 1].ip}`,
        vpcSubnets: {
          subnets: [
      ...
      zookeeper.userData.addCommands(
        ...
        `sed -i "s,server.1=.*,server.1=${props.ipsAndSubnets[0].ip}:2888:3888," /home/ubuntu/zookeeper/config/zookeeper.properties`,
        `sed -i "s,server.2=.*,server.2=${props.ipsAndSubnets[1].ip}:2888:3888," /home/ubuntu/zookeeper/config/zookeeper.properties`,
        `sed -i "s,server.3=.*,server.3=${props.ipsAndSubnets[2].ip}:2888:3888," /home/ubuntu/zookeeper/config/zookeeper.properties`,
        `cd /home/ubuntu/zookeeper`,
        `sudo bin/zookeeper-server-start.sh -daemon config/zookeeper.properties`,
        `sleep 5`,
