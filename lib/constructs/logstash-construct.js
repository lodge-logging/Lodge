...
class Logstash extends cdk.Construct {
  constructor(scope, id, props) {
    ...
    const commands = [
      ...
      `mv logstash.repo /etc/yum/repos.d/`,
      ...
      `sed -i 's/bootstrap_servers =>.*/bootstrap_servers => "${brokersIps}"/' /etc/logstash/conf.d/logstash.conf`,
      `sed -i 's/hosts =>.*/hosts => [${ESMasters}]/' /etc/logstash/conf.d/logstash.conf`,
      `sed -i 's/bucket => "bandstand-s3"/bucket => "${props.bucket.bucketName}"/' /etc/logstash/conf.d/logstash.conf`,
