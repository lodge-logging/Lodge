...
class Elasticsearch extends cdk.Construct {
  constructor(scope, id, props) {
    ...
    // data
    const dataNodeASG = new autoscaling.AutoScalingGroup(
      this,
      "Data-node-ASG",
      {
        vpc: props.vpc,
        ...
        maxCapacity: params.MaxDataNodes,
    ...
    const dataNodeCommands = [
      ...
      `curl https://bandstand-s3.s3.us-west-2.amazonaws.com/es -o /etc/elasticsearch/elasticsearch.yml`,
      `sed -i 's/node.name:.*//' /etc/elasticsearch/elasticsearch.yml`,
      `sed -i 's/node.roles:.*/node.roles: [ data ]/' /etc/elasticsearch/elasticsearch.yml`,
      `sed -i 's/discovery.ec2.endpoint:.*/discovery.ec2.endpoint: ec2.${props.env.region}.amazonaws.com/' /etc/elasticsearch/elasticsearch.yml`,
      `sudo systemctl start elasticsearch.service`,
    ];

    dataNodeASG.userData.addCommands(...dataNodeCommands);
    ...
    // SCALING METHODS
    dataNodeASG.scaleOnCpuUtilization("CpuScaling", {
