...
class KafkaStage extends cdk.Stage {
  constructor(scope, id, props) {
    ...
    const script = new StartingStack(scope, "StartingStack", {
      env: props.env,
      firstStageInstances: rana.zookeeperCluster.instances,
    });
...
class StartingStack extends cdk.Stack {
  constructor(scope, id, props) {
    ...
    console.log("=============", props.firstStageInstances);
    // const ip = props.firstStageInstances[0].instancePrivateIp;
    // props.firstStageInstances[0].userData.addCommands(`touch ${ip}`);
    ...
    test.userData.addCommands(
      `touch ${props.firstStageInstances[0].instancePrivateIp}`
    );
  }
}
