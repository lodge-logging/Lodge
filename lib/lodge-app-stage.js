  ...
  constructor(scope, id, props) {
    ...
    if (data.lodgeVpc.id) {
      ...
      lodgeVPC = new LodgeVPCStack(scope, "LodgeVPCStack", {
        env: props.env,
        existingVPC: true,
        userVPCId: data.lodgeVpc.id,
      });
      let filteredPrivateSubnets = filter(lodgeVPC.vpcStack.vpc, "private");
      let filteredPublicSubnets = filter(lodgeVPC.vpcStack.vpc, "public");
      //console.log(" lodgeVPC.vpcStack.vpc", filteredPrivateSubnets);
      lodgeVPC.vpcStack.PrivateSubnets = filteredPrivateSubnets;

      lodgeVPC.vpcStack.PublicSubnets = filteredPublicSubnets;
    } else {
      // create new vpc
      lodgeVPC = new LodgeVPCStack(scope, "LodgeVPCStack", {
        env: props.env,
